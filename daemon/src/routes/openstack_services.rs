...
use machina_core::{
    ...
    create_lb_health_monitor, create_lb_listener, create_lb_member, create_lb_pool, create_load_balancer,
    delete_heat_stack, delete_lb_health_monitor, delete_lb_listener, delete_lb_member, delete_lb_pool,
    delete_load_balancer, get_heat_stack, get_heat_stack_template, get_identity_project, get_identity_user,
    get_load_balancer, grant_role_assignment, list_heat_stack_events, list_heat_stack_resources, list_heat_stacks,
    list_identity_projects, list_identity_roles, list_identity_users, list_lb_health_monitors, list_lb_listeners,
    list_lb_members, list_lb_pools, list_load_balancers, list_role_assignments, probe_heat_reachable,
    probe_octavia_reachable, revoke_role_assignment, update_heat_stack, update_identity_user,
    CreateHeatStackRequest, CreateIdentityProjectRequest, CreateIdentityUserRequest, CreateLbHealthMonitorRequest,
    CreateLbListenerRequest, CreateLbMemberRequest, CreateLbPoolRequest, CreateLoadBalancerRequest,
    LibvirtManager, RoleAssignmentRequest, UpdateHeatStackRequest, UpdateIdentityUserRequest,
};
...
async fn os_heat_reachable() -> Result<Json<serde_json::Value>, AppError> {
    ...
    Ok(Json(serde_json::json!({ "reachable": probe_heat_reachable(&cfg).await })))
}
...
async fn os_get_heat_stack(Path((name, id)): Path<(String, String)>) -> Result<Json<serde_json::Value>, AppError> {
    let cfg = openstack_cfg();
    ensure_openstack_enabled(&cfg)?;
...
async fn os_list_heat_resources(Path((name, id)): Path<(String, String)>) -> Result<Json<serde_json::Value>, AppError> {
    let cfg = openstack_cfg();
    ensure_openstack_enabled(&cfg)?;
...
async fn os_list_heat_events(Path((name, id)): Path<(String, String)>) -> Result<Json<serde_json::Value>, AppError> {
    let cfg = openstack_cfg();
    ensure_openstack_enabled(&cfg)?;
...
async fn os_get_heat_template(Path((name, id)): Path<(String, String)>) -> Result<Json<serde_json::Value>, AppError> {
    let cfg = openstack_cfg();
    ensure_openstack_enabled(&cfg)?;
...
async fn os_create_heat_stack(Json(body): Json<CreateHeatStackRequest>) -> Result<Json<serde_json::Value>, AppError> {
    let cfg = openstack_cfg();
    ensure_openstack_enabled(&cfg)?;
...
async fn os_delete_heat_stack(Path((name, id)): Path<(String, String)>) -> Result<Json<serde_json::Value>, AppError> {
    let cfg = openstack_cfg();
    ensure_openstack_enabled(&cfg)?;
    delete_heat_stack(&cfg, &name, &id).await?;
    log_audit("openstack.heat.delete", &id, "ok");
    Ok(Json(serde_json::json!({ "status": "deleted", "stack_name": name, "id": id })))
}

async fn os_octavia_reachable() -> Result<Json<serde_json::Value>, AppError> {
    let cfg = openstack_cfg();
    ensure_openstack_enabled(&cfg)?;
    Ok(Json(serde_json::json!({ "reachable": probe_octavia_reachable(&cfg).await })))
}
...
async fn os_create_load_balancer(Json(body): Json<CreateLoadBalancerRequest>) -> Result<Json<serde_json::Value>, AppError> {
    let cfg = openstack_cfg();
    ensure_openstack_enabled(&cfg)?;
...
async fn os_delete_load_balancer(Path(id): Path<String>) -> Result<Json<serde_json::Value>, AppError> {
    let cfg = openstack_cfg();
    ensure_openstack_enabled(&cfg)?;
...
async fn os_delete_lb_listener(Path(id): Path<String>) -> Result<Json<serde_json::Value>, AppError> {
    let cfg = openstack_cfg();
    ensure_openstack_enabled(&cfg)?;
...
async fn os_create_lb_pool(Json(body): Json<CreateLbPoolRequest>) -> Result<Json<serde_json::Value>, AppError> {
    let cfg = openstack_cfg();
    ensure_openstack_enabled(&cfg)?;
...
async fn os_list_lb_members(Path(pool_id): Path<String>) -> Result<Json<serde_json::Value>, AppError> {
    let cfg = openstack_cfg();
    ensure_openstack_enabled(&cfg)?;
...
async fn os_delete_lb_member(Path((pool_id, member_id)): Path<(String, String)>) -> Result<Json<serde_json::Value>, AppError> {
    let cfg = openstack_cfg();
    ensure_openstack_enabled(&cfg)?;
    delete_lb_member(&cfg, &pool_id, &member_id).await?;
    log_audit("openstack.lb.member.delete", &member_id, "ok");
    Ok(Json(serde_json::json!({ "status": "deleted", "id": member_id })))
}

async fn os_list_lb_health_monitors(Path(pool_id): Path<String>) -> Result<Json<serde_json::Value>, AppError> {
    let cfg = openstack_cfg();
    ensure_openstack_enabled(&cfg)?;
    let healthmonitors = list_lb_health_monitors(&cfg, &pool_id).await?;
    Ok(Json(serde_json::json!({ "healthmonitors": healthmonitors })))
}
...
async fn os_delete_lb_health_monitor(Path(id): Path<String>) -> Result<Json<serde_json::Value>, AppError> {
    let cfg = openstack_cfg();
    ensure_openstack_enabled(&cfg)?;
...
async fn os_create_project(Json(body): Json<CreateIdentityProjectRequest>) -> Result<Json<serde_json::Value>, AppError> {
    let cfg = openstack_cfg();
    ensure_openstack_enabled(&cfg)?;
...
async fn os_create_user(Json(body): Json<CreateIdentityUserRequest>) -> Result<Json<serde_json::Value>, AppError> {
    let cfg = openstack_cfg();
    ensure_openstack_enabled(&cfg)?;
...
async fn os_list_role_assignments(Query(q): Query<ProjectQuery>) -> Result<Json<serde_json::Value>, AppError> {
    let cfg = openstack_cfg();
    ensure_openstack_enabled(&cfg)?;
...
async fn os_grant_role_assignment(Json(body): Json<RoleAssignmentRequest>) -> Result<Json<serde_json::Value>, AppError> {
    let cfg = openstack_cfg();
    ensure_openstack_enabled(&cfg)?;
...
async fn os_revoke_role_assignment(Json(body): Json<RoleAssignmentRequest>) -> Result<Json<serde_json::Value>, AppError> {
    let cfg = openstack_cfg();
    ensure_openstack_enabled(&cfg)?;
...
pub fn openstack_services_routes() -> Router<LibvirtManager> {
    Router::new()
        ...
        .route("/openstack/heat/stacks", get(os_list_heat_stacks).post(os_create_heat_stack))
        .route(
            "/openstack/heat/stacks/{name}/{id}",
            get(os_get_heat_stack).patch(os_update_heat_stack).delete(os_delete_heat_stack),
        )
        .route("/openstack/heat/stacks/{name}/{id}/resources", get(os_list_heat_resources))
        .route("/openstack/heat/stacks/{name}/{id}/events", get(os_list_heat_events))
        .route("/openstack/heat/stacks/{name}/{id}/template", get(os_get_heat_template))
        ...
        .route("/openstack/load-balancers/listeners/{id}", delete(os_delete_lb_listener))
        .route(
            "/openstack/load-balancers/{id}/pools",
            get(os_list_lb_pools),
        )
        .route("/openstack/load-balancers/pools", post(os_create_lb_pool))
        .route("/openstack/load-balancers/pools/{id}", delete(os_delete_lb_pool))
        .route(
        ...
        .route("/openstack/load-balancers/health-monitors", post(os_create_lb_health_monitor))
        .route("/openstack/load-balancers/health-monitors/{id}", delete(os_delete_lb_health_monitor))
        .route("/openstack/identity/projects", get(os_list_projects).post(os_create_project))
        .route("/openstack/identity/projects/{id}", get(os_get_project))
        .route("/openstack/identity/users", get(os_list_users).post(os_create_user))
        .route("/openstack/identity/users/{id}", get(os_get_user).put(os_update_user))
        .route("/openstack/identity/roles", get(os_list_roles))
        .route("/openstack/identity/role-assignments", get(os_list_role_assignments).put(os_grant_role_assignment))
        .route("/openstack/identity/role-assignments/revoke", post(os_revoke_role_assignment))
