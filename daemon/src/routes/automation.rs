...
async fn list_roles(State(_m): State<LibvirtManager>) -> Result<Json<serde_json::Value>, AppError> {
    ...
    let list: Vec<_> = roles.into_iter().map(|(u, r)| serde_json::json!({"username": u, "role": r})).collect();
    Ok(Json(serde_json::json!(list)))
}

#[derive(Deserialize)]
struct SetRoleRequest { username: String, role: automation::Role }

async fn set_role(State(_m): State<LibvirtManager>, Json(req): Json<SetRoleRequest>) -> Result<Json<serde_json::Value>, AppError> {
    automation::set_user_role(&req.username, req.role.clone())?;
    Ok(Json(serde_json::json!({"status": "ok", "username": req.username, "role": req.role})))
}

// ── API Tokens ─────────────────────────────────────────────────────

async fn list_tokens(State(_m): State<LibvirtManager>) -> Result<Json<serde_json::Value>, AppError> {
    let tokens = automation::list_api_tokens();
    Ok(Json(serde_json::json!(tokens)))
}

#[derive(Deserialize)]
struct CreateTokenRequest { name: String, username: String, role: automation::Role }

async fn create_token(State(_m): State<LibvirtManager>, Json(req): Json<CreateTokenRequest>) -> Result<Json<serde_json::Value>, AppError> {
    let token = automation::create_api_token(&req.name, &req.username, req.role)?;
    Ok(Json(serde_json::json!(token)))
}

async fn delete_token(State(_m): State<LibvirtManager>, Path(token): Path<String>) -> Result<Json<serde_json::Value>, AppError> {
...
async fn list_alert_rules(State(_m): State<LibvirtManager>) -> Result<Json<serde_json::Value>, AppError> {
    let rules = automation::load_alert_rules();
    Ok(Json(serde_json::json!(rules)))
}

async fn save_alert_rules_handler(State(_m): State<LibvirtManager>, Json(rules): Json<Vec<automation::AlertRule>>) -> Result<Json<serde_json::Value>, AppError> {
    automation::save_alert_rules(&rules)?;
    Ok(Json(serde_json::json!({"status": "ok"})))
}

async fn list_alerts(State(_m): State<LibvirtManager>) -> Result<Json<serde_json::Value>, AppError> {
    let alerts = automation::load_alerts();
    Ok(Json(serde_json::json!(alerts)))
}

async fn acknowledge_alert_handler(State(_m): State<LibvirtManager>, Path(id): Path<String>) -> Result<Json<serde_json::Value>, AppError> {
...
async fn list_webhooks(State(_m): State<LibvirtManager>) -> Result<Json<serde_json::Value>, AppError> {
    let hooks = automation::load_webhooks();
    Ok(Json(serde_json::json!(hooks)))
}

async fn save_webhooks_handler(State(_m): State<LibvirtManager>, Json(hooks): Json<Vec<automation::WebhookConfig>>) -> Result<Json<serde_json::Value>, AppError> {
...
async fn list_schedules(State(_m): State<LibvirtManager>) -> Result<Json<serde_json::Value>, AppError> {
    let schedules = automation::load_schedules();
    Ok(Json(serde_json::json!(schedules)))
}

async fn save_schedules_handler(State(_m): State<LibvirtManager>, Json(schedules): Json<Vec<automation::ScheduledAction>>) -> Result<Json<serde_json::Value>, AppError> {
...
pub fn automation_routes() -> Router<LibvirtManager> {
    Router::new()
        ...
        // Alerts
        .route("/alert-rules", get(list_alert_rules).post(save_alert_rules_handler))
