#!/usr/bin/env node
/**
 * Title/caption cards for Machina GuestKit agent live UX reel.
 */
...
const outDir = join(__dirname, 'png-guestkit');
...
const cards = [
  { file: 'gk00-title', kicker: 'MACHINA × GUESTKIT', line1: 'Linux Guest Agent, Live from the Control Plane', line2: 'guest-ping · TRIM · set IP/routes · start/stop services' },
  { file: 'gk01-login', kicker: '', line1: 'Sign In to Platform' },
  { file: 'gk02-health', kicker: '', line1: 'Guest Health — Agent Connected' },
  { file: 'gk02b-network', kicker: '', line1: 'Set Guest IP & Gateway' },
  { file: 'gk03-services', kicker: '', line1: 'Start & Stop Guest Services' },
  { file: 'gk04-outro', kicker: '', line1: 'GuestKit agent speaks QGA — Machina drives it.', line2: 'zyvor.dev/machina · zyvor.dev/guestkit' },
];

const captions = [
  { file: 'cap-gk-login', text: 'One login — the same session reaches every hypervisor and every guest agent.' },
  { file: 'cap-gk-health', text: 'Guest health: channel connected, guestkit-agent answering guest-ping over virtio-serial.' },
  { file: 'cap-gk-sync', text: 'TRIM guest filesystems — a real QGA fstrim action from the UX.' },
  { file: 'cap-gk-network', text: 'Detect netplan / NetworkManager / networkd, then apply IP + gateway inside the guest.' },
  { file: 'cap-gk-stop', text: 'Stop a controllable guest unit (machina-demo) via guest-exec systemctl.' },
  { file: 'cap-gk-start', text: 'Start it again — inventory refreshes live from the agent.' },
];
