...
/**
 * CDP UI: planner shells — blueprints, vm-builder, create, observability, SOC.
 */
...
const log = createLogger(cfg.resultsDir, 'ui-planner');
...
async function loadPath(cdp, path) {
  const wait = path.startsWith('/platform') || path === '/create' ? 14000 : 10000;
...
(async () => {
  ...
  console.log(`UI_PLANNER_DONE pass=${pass} soft=${soft} fail=${fail}`);
