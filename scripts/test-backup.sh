...
COUNT=$(curl -sf "$API/backups" | python3 -c "import json,sys; print(len(json.load(sys.stdin)))")
check "2 backups exist" "2" "$COUNT"
FIRST_ID=$(curl -sf "$API/backups" | python3 -c "import json,sys; print(json.load(sys.stdin)[0]['id'])")
check "Newest first" "$BID2" "$FIRST_ID"
echo ""

# ── Test 4: Metadata fields ─────────────────────────────────
echo "--- Test 4: Backup metadata ---"
META=$(curl -sf "$API/backups" | python3 -c "import json,sys; b=json.load(sys.stdin)[1]; print(json.dumps(b))")
...
VERIFY=$(curl -sf -X POST "$API/backups/$BID1/verify")
...
TARSIZE=$(curl -sf "$API/backups/$BID1/download" | wc -c)
...
SCHED=$(curl -sf "$API/backups/schedule")
check "Timer is installed" "True" "$(echo "$SCHED" | python3 -c "import json,sys; print(json.load(sys.stdin)['installed'])")"

RESP=$(curl -sf -X POST "$API/backups/schedule" -H 'Content-Type: application/json' -d '{"enabled":true}')
...
RESP=$(curl -sf -X POST "$API/backups/schedule" -H 'Content-Type: application/json' -d '{"enabled":false}')
check "Disable timer" "disable" "$RESP"
echo ""

# ── Test 8: Restore ─────────────────────────────────────────
echo "--- Test 8: Restore ---"
RESP=$(curl -sf -X POST "$API/backups/restore" -H 'Content-Type: application/json' -d "{\"backup_id\":\"$BID1\"}")
...
RESP=$(curl -sf -X POST "$API/backups" -H 'Content-Type: application/json' -d '{"vm_name":"photon-test-vm","with_disks":true}')
...
for i in $(seq 1 60); do
    STAT=$(curl -sf "$API/backups/$BID3/status" 2>/dev/null | python3 -c "import json,sys; print(json.load(sys.stdin)['status'])" 2>/dev/null || echo "pending")
...
STATUS=$(curl -sf "$API/backups/$BID3/status")
check "Disk backup completed" "completed" "$STATUS"
SIZE=$(curl -sf "$API/backups" | python3 -c "import json,sys; [print(b['size']) for b in json.load(sys.stdin) if b['id']=='$BID3']")
...
echo "--- Test 11: Delete ---"
RESP=$(curl -sf -X DELETE "$API/backups/$BID3")
...
VERIFY=$(curl -sf -X POST "$API/backups/$BID2/verify")
