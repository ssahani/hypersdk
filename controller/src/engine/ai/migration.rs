...
pub fn merge_guestkit(
    ...
    for b in blockers {
        ...
        report.remediation.push("Run GuestKit repair / migrate-plan export before cutover".into());
    }
    for w in warnings.iter().take(3) {
        report.risks.push(format!("GuestKit warning: {w}"));
    }
    if migration_score >= 80.0 {
        report.safe.push(format!("GuestKit migration score {:.0}% on KVM target", migration_score));
    }
    report
}
...
pub fn merge_firewall_migration(
    ...
    if dependencies.is_empty() {
        report.firewall_migration_summary =
            Some("No observed firewall port dependencies — verify with PacketWolf before cutover".into());
        return report;
    }
    ...
    report.remediation.push(
        "Apply matching Zeus Firewall profile on target VM before migration cutover".into(),
    );
