...
pub async fn list_networks(
    ...
    let rows = sqlx::query_as::<_, NetworkRow>(
        "SELECT id, name, backend, vlan_id, bridge FROM networks ORDER BY name",
    )
...
pub async fn create_network(
    ...
    let id = Uuid::new_v4();
    sqlx::query(
        "INSERT INTO networks (id, cluster_id, name, backend, vlan_id, bridge)
         VALUES ($1, $2, $3, $4, $5, $6)",
    )
    .bind(id)
    .bind(cluster_id)
    ...
    let row = sqlx::query_as::<_, NetworkRow>(
        "SELECT id, name, backend, vlan_id, bridge FROM networks WHERE id = $1",
    )
    .bind(id)
    .fetch_one(&state.pool)
    .await?;
