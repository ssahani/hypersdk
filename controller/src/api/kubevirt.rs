...
pub async fn sync_inventory(
    ...
    let cluster_id: Uuid = sqlx::query_scalar("SELECT id FROM clusters ORDER BY created_at LIMIT 1")
    ...
    Ok(Json(KubeVirtSyncResponse {
        ...
        message: "KubeVirt VMs reconciled with platform inventory (VMware/Proxmox remain import-only)".into(),
    }))
}
