...
### One-command install (machina + optional Guacamole)

Built-in noVNC is installed with `install.sh` (`novnc` + `spice-html5` packages). Optional Apache Guacamole:

```bash
sudo ./install.sh --with-guacamole
sudo ./install.sh --bind 0.0.0.0 --open-firewall --with-guacamole
sudo bash scripts/install-guacamole.sh          # stack only (after machina)
...
```bash
./scripts/deploy-remote.sh USER@HOST --with-guacamole --bind 0.0.0.0 --open-firewall
```

Secrets and compose env live in `/etc/machina/guacamole.env` (mode 600). The installer writes matching `[guacamole]` into `/etc/machina/config.toml`.

**`/etc/machina/config.toml`** (auto-generated by `scripts/install-guacamole.sh`; `json_secret_hex` must match `JSON_SECRET_KEY` in `guacamole.env`):
