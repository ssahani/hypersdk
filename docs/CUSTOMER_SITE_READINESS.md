...
| Windows guest RDP-first | Lab golden `win10-msedge`: feature-test **26/26**; waves 2–3 API+UI green (platform through security/provision/mission/catalog); RDP refuse-while-running **PASS**; offline guestkit enable-rdp still **500** (0 registry ops) — rely on hyper2kvm firstboot until GuestKit NTFS write path confirmed |
