    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={load} className="p-2 hover:bg-gray-700 rounded transition" aria-label="Refresh"><RefreshCw className="w-4 h-4" /></button>
        </div>
      </div>
          ...
          <tbody className="divide-y divide-gray-700">
            {networks.map((net) => (
              <tr key={net.name} className="hover:bg-gray-700/50">
                ...
                <td className="px-6 py-3 hidden md:table-cell text-sm text-gray-400 font-mono">{net.bridge || '-'}</td>
            ...
            {networks.length === 0 && <tr><td colSpan={5} className="px-6 py-8 text-center text-gray-500">No networks</td></tr>}
          </tbody>
        </table>
      </div>
      ...
      {leases.length > 0 && (
        <div className="bg-gray-800 rounded-lg border border-gray-700 overflow-hidden">
          <div className="px-6 py-3 border-b border-gray-700"><h2 className="text-sm font-semibold text-gray-300">DHCP Leases</h2></div>
          <table className="w-full">
            <thead><tr className="border-b border-gray-700 text-left text-xs text-gray-500"><th className="px-6 py-2">Network</th><th className="px-6 py-2">IP Address</th><th className="px-6 py-2">MAC</th><th className="px-6 py-2">Hostname</th><th className="px-6 py-2">Expires</th></tr></thead>
            <tbody className="divide-y divide-gray-700 text-sm">
              {leases.map((l, i) => (
                <tr key={i} className="table-row-hover">
                  <td className="px-6 py-2 text-gray-400">{l.network}</td>
                  <td className="px-6 py-2 font-mono text-blue-400">{l.ip}</td>
                  <td className="px-6 py-2 font-mono text-xs text-gray-400">{l.mac}</td>
                  <td className="px-6 py-2">{l.hostname || '-'}</td>
                  <td className="px-6 py-2 text-gray-400">{l.expiry}</td>
                </tr>
              ))}
            </tbody>
          </table>
