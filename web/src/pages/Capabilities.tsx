import { useEffect, useState, useCallback } from 'react'
import { getCapabilities, getSysinfo, CapabilitiesInfo } from '../api/advanced'
import { useToastContext } from '../contexts/ToastContext'
import { RefreshCw, Cpu, Info } from 'lucide-react'

export default function CapabilitiesPage() {
  const [capabilities, setCapabilities] = useState<CapabilitiesInfo | null>(null)
  const [sysinfo, setSysinfo] = useState('')
  const [tab, setTab] = useState<'capabilities' | 'sysinfo'>('capabilities')
  const [loading, setLoading] = useState(true)
  const toast = useToastContext()

  const load = useCallback(async () => {
    try {
      const [caps, sys] = await Promise.all([
        getCapabilities().catch(() => null),
        getSysinfo().catch(() => ''),
      ])
      setCapabilities(caps)
      setSysinfo(sys)
    } catch (e: unknown) {
      toast.error(`${e instanceof Error ? e.message : e}`)
    } finally {
      setLoading(false)
    }
  }, [toast])

  useEffect(() => { load() }, [load])

  if (loading) return <div className="flex items-center justify-center h-32"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-500" /></div>

  const tabs = [
    { key: 'capabilities' as const, label: 'Capabilities', icon: <Cpu className="w-4 h-4" /> },
    { key: 'sysinfo' as const, label: 'System Info', icon: <Info className="w-4 h-4" /> },
  ]

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold flex items-center gap-2"><Cpu className="w-6 h-6" /> Capabilities</h1>
        <button onClick={load} className="p-2 hover:bg-gray-700 rounded transition"><RefreshCw className="w-4 h-4" /></button>
      </div>

      <div className="flex gap-1 border-b border-gray-700">
        {tabs.map((t) => (
          <button key={t.key} onClick={() => setTab(t.key)} className={`flex items-center gap-2 px-4 py-2 text-sm transition border-b-2 ${tab === t.key ? 'border-blue-500 text-blue-400' : 'border-transparent text-gray-400 hover:text-gray-200'}`}>
            {t.icon} {t.label}
          </button>
        ))}
      </div>

      {tab === 'capabilities' && capabilities && (
        <div className="space-y-6">
          <div className="bg-gray-800 rounded-lg p-6 border border-gray-700 space-y-4">
            <h3 className="text-lg font-semibold">Host</h3>
            <div className="flex items-center justify-between py-2 border-b border-gray-700/50">
              <span className="text-gray-400 text-sm">Architecture</span>
              <span className="text-sm font-medium">{capabilities.host_arch}</span>
            </div>
            <div className="flex items-center justify-between py-2 border-b border-gray-700/50">
              <span className="text-gray-400 text-sm">CPU Model</span>
              <span className="text-sm font-medium">{capabilities.host_cpu_model}</span>
            </div>
          </div>

          <div className="bg-gray-800 rounded-lg border border-gray-700 overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-700">
              <h3 className="text-lg font-semibold">Guest Architectures</h3>
            </div>
            {capabilities.guests.length === 0 ? <div className="p-8 text-center text-gray-500">No guest capabilities.</div> : (
              <table className="w-full">
                <thead><tr className="border-b border-gray-700 text-left text-sm text-gray-400"><th className="px-6 py-3">OS Type</th><th className="px-6 py-3">Architecture</th><th className="px-6 py-3">Machines</th></tr></thead>
                <tbody className="divide-y divide-gray-700">
                  {capabilities.guests.map((g, i) => (
                    <tr key={i} className="hover:bg-gray-700/50">
                      <td className="px-6 py-3 text-sm font-medium">{g.os_type}</td>
                      <td className="px-6 py-3 text-sm">{g.arch}</td>
                      <td className="px-6 py-3 text-sm text-gray-400">{g.machines.slice(0, 5).join(', ')}{g.machines.length > 5 ? ` (+${g.machines.length - 5} more)` : ''}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            )}
          </div>
        </div>
      )}

      {tab === 'capabilities' && !capabilities && (
        <div className="bg-gray-800 rounded-lg border border-gray-700 p-8 text-center text-gray-500">No capabilities data available.</div>
      )}

      {tab === 'sysinfo' && (
        <div className="bg-gray-800 rounded-lg border border-gray-700 overflow-hidden">
          <pre className="p-6 text-sm text-gray-300 overflow-x-auto whitespace-pre-wrap font-mono">
            {sysinfo || 'No system info available.'}
          </pre>
        </div>
      )}
    </div>
  )
}
