      ...
      <div className="relative" onClick={(e) => e.stopPropagation()}>
        ...
        {open && (
          <div className="absolute left-0 top-full mt-1 z-50 min-w-[12rem] rounded-xl border border-white/[0.08] bg-slate-900/95 shadow-xl py-1">
            <button
              type="button"
              className={`block w-full text-left px-3 py-2 text-xs hover:bg-white/[0.04] ${!workspace ? 'text-violet-200' : 'text-slate-300'}`}
            ...
            {workspaces.map((w) => (
              <button
                key={w.name}
                type="button"
                className={`block w-full text-left px-3 py-2 text-xs hover:bg-white/[0.04] ${workspace === w.name ? 'text-violet-200' : 'text-slate-300'}`}
                onClick={() => pickWorkspace(w.name)}
              >
                {w.name} <span className="text-slate-500">({w.vm_count})</span>
              </button>
            ))}
            <Link
              to="/platform/users?tab=workspaces"
              className="block px-3 py-2 text-xs text-blue-400 border-t border-white/[0.06] mt-1 hover:bg-white/[0.04]"
