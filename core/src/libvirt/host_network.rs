...
pub fn list_host_interfaces() -> Result<Vec<HostInterface>, LibvirtError> {
    ...
    if let Some(arr) = json.as_array() {
        for iface in arr {
            ...
            let state = if iface["operstate"].as_str().unwrap_or("") == "UP" { "up" } else { "down" };
...
fn classify_interface(name: &str, link_type: &str, iface: &serde_json::Value) -> &'static str {
    if link_type == "bridge" { return "bridge"; }
    if link_type == "bond" { return "bond"; }
    if name.contains('.') || iface.get("link").is_some() { return "vlan"; }
    if name.starts_with("vnet") || name.starts_with("tap") || name.starts_with("veth")
        || name.starts_with("virbr") || name.starts_with("docker") { return "virtual"; }
    "physical"
}
...
pub fn create_bridge(req: &CreateBridgeRequest) -> Result<(), LibvirtError> {
    ...
    if req.mtu < 68 || req.mtu > 9216 {
        return Err(LibvirtError::Invalid("MTU must be between 68 and 9216".to_string()));
    }

    // Validate interface names
    for iface in &req.interfaces {
        if !iface.chars().all(|c| c.is_alphanumeric() || c == '-' || c == '_' || c == '.') {
            return Err(LibvirtError::Invalid(format!("Invalid interface name: {iface}")));
        }
    }
...
fn create_bridge_nmcli(req: &CreateBridgeRequest) -> Result<(), LibvirtError> {
    ...
    run_cmd("nmcli", &[
        ...
        "con-name", &req.name,
        "bridge.stp", stp,
        "802-3-ethernet.mtu", &req.mtu.to_string(),
    ...
    for (i, iface) in req.interfaces.iter().enumerate() {
        ...
        run_cmd("nmcli", &[
            "connection", "add", "type", "bridge-slave",
            "ifname", iface,
            "con-name", &slave_name,
            "master", &req.name,
...
fn create_bridge_ip(req: &CreateBridgeRequest) -> Result<(), LibvirtError> {
    run_cmd("ip", &["link", "add", "name", &req.name, "type", "bridge"], "Failed to create bridge")?;
    run_cmd("ip", &["link", "set", &req.name, "mtu", &req.mtu.to_string()], "MTU set")?;
    ...
    for iface in &req.interfaces {
        run_cmd("ip", &["link", "set", iface, "master", &req.name], &format!("Failed to add {iface}"))?;
    }

    run_cmd("ip", &["link", "set", &req.name, "up"], "Failed to bring bridge up")?;
    Ok(())
}
...
pub fn delete_bridge(name: &str) -> Result<(), LibvirtError> {
    ...
    if Command::new(find_bin("nmcli")).arg("--version").output().is_ok() {
        ...
        if let Ok(output) = Command::new(find_bin("nmcli")).args(["-t", "-f", "NAME,DEVICE", "connection", "show"]).output() {
        ...
        run_cmd("nmcli", &["connection", "delete", name], "Failed to delete bridge")
    } else {
        run_cmd("ip", &["link", "set", name, "down"], "bridge down")?;
        run_cmd("ip", &["link", "delete", name, "type", "bridge"], "Failed to delete bridge")
    }
}
...
pub fn list_port_forwards() -> Result<Vec<PortForwardRule>, LibvirtError> {
    ...
    let output = Command::new(find_bin("iptables"))
        .args(["-t", "nat", "-L", "PREROUTING", "-n", "--line-numbers", "-v"])
        .output()
...
pub fn create_port_forward(req: &CreatePortForwardRequest) -> Result<(), LibvirtError> {
    ...
    if req.vm_ip.parse::<std::net::Ipv4Addr>().is_err() {
        return Err(LibvirtError::Invalid(format!("Invalid VM IP: {}", req.vm_ip)));
    }
    ...
    run_cmd("iptables", &[
        "-t", "nat", "-A", "PREROUTING",
        "-p", &req.protocol,
        "--dport", &req.host_port.to_string(),
        "-j", "DNAT",
        "--to-destination", &format!("{}:{}", req.vm_ip, req.vm_port),
...
pub fn delete_port_forward(proto: &str, host_port: u16, vm_ip: &str, vm_port: u16) -> Result<(), LibvirtError> {
    ...
    let _ = Command::new(find_bin("iptables"))
        .args([
            ...
            "-j", "DNAT",
            "--to-destination", &format!("{vm_ip}:{vm_port}"),
        ])
        .output();
...
pub fn list_firewall_rules() -> Result<Vec<FirewallRule>, LibvirtError> {
    ...
    for line in stdout.lines() {
        if !line.contains("vs-fw:") { continue; }

        let action = if line.contains("ACCEPT") { "accept" } else if line.contains("DROP") { "drop" } else { continue };
        let proto = if line.contains(" tcp ") { "tcp" } else if line.contains(" udp ") { "udp" } else if line.contains(" icmp ") { "icmp" } else { "all" };
        ...
        rules.push(FirewallRule {
            id, vm_ip, direction, protocol: proto.to_string(),
            port, action: action.to_string(), description: desc,
        });
    }
...
pub fn create_firewall_rule(req: &CreateFirewallRuleRequest) -> Result<(), LibvirtError> {
    ...
    if req.direction != "inbound" && req.direction != "outbound" {
        return Err(LibvirtError::Invalid("Direction must be 'inbound' or 'outbound'".to_string()));
    }
    if req.action != "accept" && req.action != "drop" {
        return Err(LibvirtError::Invalid("Action must be 'accept' or 'drop'".to_string()));
    }
    let valid_protos = ["tcp", "udp", "icmp", "all"];
    if !valid_protos.contains(&req.protocol.as_str()) {
        return Err(LibvirtError::Invalid(format!("Protocol must be one of: {}", valid_protos.join(", "))));
    }

    let target = if req.action == "accept" { "ACCEPT" } else { "DROP" };
    let desc = req.description.chars().filter(|c| c.is_alphanumeric() || *c == ' ' || *c == '-' || *c == '_').collect::<String>();
